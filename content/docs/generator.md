...
The Knowledge Graph Explorer renders a payload the generator assembles. After `Build()` writes `graph.json`, `meta.json`, and `backlinks.json`, the generator invokes `internal/graphexplorer.Write()`, which renders `<output>/graph/index.html` from an embedded HTML template (`internal/graphexplorer/assets/template.html`) and writes `<output>/graph/data.json` beside it. The template references the companion client bundle (`<output>/assets/graph/explorer.{js,css}`); the asset pipeline prefers files from the selected project and fills missing runtime files from the release-owned embedded bundle.
