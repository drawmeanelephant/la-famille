...
- **Action:** Integrate a YAML frontmatter parser to extract metadata like `Title`, `Date`, `Author`, and `Draft` status from the top of the `.md` files before rendering with Goldmark.
...
---
**Note to Jules:** When picking up tasks from this list, remember to review the current `GEMINI.md` and update `learnings.txt` with any new challenges or architectural decisions encountered!
