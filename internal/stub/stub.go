...
func GenerateStubs(cfg config.Config, missingFiles map[string][]string, g *graph.Graph, p *bluemonday.Policy) error {
	...
	for _, missingRelPath := range missingKeys {
		...
		files := []string{cfg.Template}
		matches, _ := filepath.Glob(filepath.Join(filepath.Dir(cfg.Template), "partials", "*.html"))
		if len(matches) > 0 {
			files = append(files, matches...)
		} else {
			dir, _ := os.Getwd()
			for {
				globPath := filepath.Join(dir, "templates", "partials", "*.html")
		...
		defaultTmpl, err := template.ParseFiles(files...)
		if err != nil {
			outFile.Close()
